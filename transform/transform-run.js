...
ON('flow.ready', function() {
	FLOWOPTIONS({ parser: 'xml' });
	FLOWDATA('<CD><TITLE>A</TITLE><PRICE>10.00</PRICE></CD>')
	FLOWOPTIONS({ parser: 'json' });
	FLOWDATA('{"title":"A","price":10.00}');
	FLOWOPTIONS({ parser: 'custom', fn: 'next(value.toLowerCase())' });
	FLOWDATA('VALUE');
	FLOWCLOSE();
});
